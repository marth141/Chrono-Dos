...
function prop_DateCleaner(propBacklog) {
  ...
  var phaseSSComp, OpPropStatDate, ssExtComp;
  if (validateHeader('Opportunity: Proposal Requested', backlogArray, dim) &&
    ...
    phaseSSComp = getMeThatColumn('Phase: Site Survey Completed', backlogArray, dim);
    OpPropStatDate = getMeThatColumn('Opportunity: Proposal Status Date', backlogArray, dim);
    ssExtComp = getMeThatColumn('Phase: Site Survey Exterior Completed', backlogArray, dim);
  ...
  var dateAdjLog = prop_RemoveLateDates(backlogArray, dim, phaseSSComp, OpPropStatDate, ssExtComp);
  prop_SortAndCleanDates(propBacklog, dateAdjLog, dim, phaseSSComp, OpPropStatDate);
}
