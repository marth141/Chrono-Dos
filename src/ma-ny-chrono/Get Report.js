function getReport() {
  ...
  completeBacklog = range.getValues().filter(function(row,index) { 
    ...
    var foundStateBool = ["fl-", "ma-", "nh-", "nj-", "ny-", "pa-", "ri-", "sc-", "vt-", "hi-"].some(function(state) { return row[officeCol].toLowerCase().indexOf(state) > -1 });
    var foundNYBool = ["ny-09"].some(function(state) { return row[officeCol].toLowerCase().indexOf(state) > -1 || row[notesCol].toLowerCase().indexOf(state) > -1 });
