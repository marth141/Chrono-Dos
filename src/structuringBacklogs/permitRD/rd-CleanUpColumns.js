...
function rd_CleanUpColumns(backlogArray) {
  ...
  var projectIDCol = getColumnIndex("Solar Project ID", backlogArray);
  var cadIDCol = getColumnIndex("Solar CAD ID", backlogArray);
  var recordTypeCol = getColumnIndex("Record Type", backlogArray);
  var primaryReasonCol = getColumnIndex("Redesign Primary Reason", backlogArray);
  var permitCompletedByCol = getColumnIndex("Primary CAD: Permit Packet Completed By", backlogArray);
  // Put index"s to remove in array and sort to insure order, remove from end (right to left) to not interupt index's
  var removeValFromIndex = [projectIDCol, cadIDCol, recordTypeCol, primaryReasonCol, permitCompletedByCol].sort(function (a, b) { return b - a; });
