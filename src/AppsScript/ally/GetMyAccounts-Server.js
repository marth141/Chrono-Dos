...
function getMyAccounts(designer) {
  ...
  if (designer.dept === 'CP') var chronoIds = cpChronoIds;
  else var chronoIds = ppChronoIds;
  for (var c in chronoIds) {
    ...
    if (designer.master) {
      ss.getRange(3, regionCol + 1, ss.getLastRow(), lastUpdateCol + 1)
        .getValues()
        .filter(function(value) {
          // Find accounts in designers name
          if (
            value[assignedCol] !== '' &&
            (designer.allNames.indexOf(value[assignedCol]) > -1 ||
              designer.allsfNames.indexOf(value[assignedCol]) > -1)
          ) {
        ...
        .filter(function(value) {
          ...
          if (
            (value[assignedCol]
              .toString()
              .toLowerCase()
              .indexOf(designer.name.toLowerCase()) > -1 ||
              value[assignedCol].toString().toLowerCase() == designer.sfName.toLowerCase()) &&
            value[assignedCol] !== ''
          ) {
            value[dueDateCol] = dateFormat(value[dueDateCol]);
            value[lastUpdateCol] = dateFormat(value[lastUpdateCol]);
