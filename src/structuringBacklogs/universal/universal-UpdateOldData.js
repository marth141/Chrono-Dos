...
function uni_UpdateOldData(FilterSettings, backlogArray, oldData) {
  ...
  if (serviceCol === -1) {
    serviceCol = getMeThatColumnNoValidate(
 ...
 * @param {*} notesCol
 * @param {*} lastUpdateCol
 * @param {*} initialUpdateCol
 * @return {*} backlogArray
 */
function replaceOldInfo(
  FilterSettings,
  backlogArray,
  oldData,
  serviceCol,
  ...
  for (var oldRow = 0; oldRow < oldData.length; oldRow++) {
    for (var newRow = 0; newRow < backlogArray.length; newRow++) {
      if (
        !checkServiceNumbers(
          oldData[oldRow][serviceCol],
          backlogArray[newRow][serviceCol]
        )
      ) {
        //Check service numbers are the same, if not continue
        continue;
      }
      ...
      if (
        checkUnitType(
          oldData[oldRow][unitTypeCol],
          backlogArray[newRow][unitTypeCol]
        )
      ) {
        //Check unit type
        ...
        if (checkServiceNumber === backlogArray[newRow][serviceCol]) {
          console.error(
            'Here it is @ update Check Unit Type post SR After assignment!'
          );
          console.log(backlogArray[newRow]);
          console.log(backlogArray[oldRow]);
        }
      ...
      if (
        oldData[oldRow][unitTypeCol] !== '' &&
        oldData[oldRow][unitTypeCol] !== backlogArray[newRow][unitTypeCol]
      ) {
        continue;
      }
      ...
      backlogArray[newRow][initialUpdateCol] =
        oldData[oldRow][initialUpdateCol]; // Get old initial update and update to new sheet
      if (
        checkSRAssigned(
          backlogArray[newRow][unitTypeCol],
...
function checkUnitType(oldUnitType, currentUnitType) {
  if (
    currentUnitType === 'SR' &&
    oldUnitType !== 'PERMIT RD' &&
    oldUnitType !== 'DE RD'
  ) {
    return true;
  }

  var check1 = ['PERMIT RD', 'DE RD', 'CP MATCH', 'SR'].some(function(value) {
    return currentUnitType.indexOf(value) > -1;
  });
  var check2 = ['PERMIT RD', 'DE RD', 'CP MATCH', 'SR'].some(function(value) {
    return oldUnitType.indexOf(value) > -1;
  });
  return (
    oldUnitType !== currentUnitType && oldUnitType !== '' && !(check1 || check2)
  );
}
...
function checkSRAssigned(unitType, assigned, FilterSettings) {
  if (unitType === 'SR') {
    var foundUser = getUsers(FilterSettings).some(function(user) {
      return user[0] === assigned || user[2] === assigned;
    });
    if (!foundUser) {
      return true;
    }
