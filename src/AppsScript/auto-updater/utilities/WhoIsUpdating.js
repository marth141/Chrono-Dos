...
/**
 * Used to set up the backlog that's being updated.
 * @return {String}
 */
function getBacklog2Update() {
  if (updateJSON.teamKeyChain.normPlus === 'Normal') {
    // Prod Chrono
    return SpreadsheetApp.openById(updateJSON.teamKeyChain.chrono).getSheetByName(
      updateJSON.identity.sheetNames.normal
    );
    ...
    // Prod CHrono
    return SpreadsheetApp.openById(updateJSON.teamKeyChain.chrono).getSheetByName(
      updateJSON.identity.sheetNames.plus
    );
...
function getBacklogName(inputSheet) {
  ...
  if (backlogPasteID.match(/Solar$/g)) {
    ...
    inputIdentity = {
      'team': inputIdentity[0][0],
      'sheetNames': {
        'plus': inputIdentity[1][0] + ' BACKLOG',
        'normal': inputIdentity[1][0]
      }
    };

    // If Solar Console
  } else if (backlogPasteID.match(/Solar Console$/g)) {
    inputIdentity = inputSheet
      .getRange('A5:A6')
      .getValues()
      .join()
      .replace(',', ' ');
...
/**
 * Determines the spreadsheet key of the backlog
 * @return {*}
 */
function team_determiner() {
  if (updateJSON.identity.team.match(/CP DIN/i)) {
    ...
    return (updateJSON.teamKeyChain = {
      'chrono': '1jvUx8XUuTFvHgDDk9CB21ZwLsI8jCIHhMIKHZ27flLw',
      'devChrono': undefined,
      'normPlus': 'Normal'
    });
