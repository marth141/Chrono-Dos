/**
 * Used for the time studies tracker
 * @param {*} accountInfo
 * @param {*} designer
 */
function timeTracker(accountInfo, designer) {
  ...
  var data = [
    [
      designer.sfName,
      accountInfo.serviceNumber,
      accountInfo.spNumber,
      accountInfo.unitType,
      accountInfo.office,
      new Date()
    ]
  ];
...
function getTimeStudiesId(dept, team) {
  var outsource;
  if (team.match(/outsource/i)) var outsource = true;
  else var outsource = false;
  if (dept.match(/cp/i) && !team.match(/outsource/i)) {
    ...
    var timeStudiesErrMsg = 'Missing Time Studies ID for DEPT: ' + dept + ' TEAM: ' + team;
    throw timeStudiesErrMsg;
  }
}

/**
 * Used to test time studies
 */
