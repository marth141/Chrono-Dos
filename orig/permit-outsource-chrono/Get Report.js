function getReport() {
  ...
  var completeBacklog = [];
  var outsourceTeam = ssFilterSettings.getRange("B6:D").getValues().filter(function(row) { return row[0] !== "" || row[2] !== ""; });
  ...
  completeBacklog = range.getValues().filter(function(row,index) { 
    ...
    var foundMemeberBool = outsourceTeam.some(function(state) { return row[assignedCol].toLowerCase().indexOf(state[0].toLowerCase()) > -1 || row[assignedCol].toLowerCase().indexOf(state[2].toLowerCase()) > -1 });
    ...
    if((foundMemeberBool || (outsourceBool && unassigned) || (otsBool && unassigned)) && !onHoldBool) {
...
//  completeBacklog = sortBacklog(completeBacklog);
