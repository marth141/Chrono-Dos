/**
 * Used to update reports manually.
 */
function updateReport() {
  ...
  if (lock.hasLock()) {
    ...
    for (var row in inputRawData) {
      if (
        inputRawData[row][0].match(/permit$/i) ||
    ...
    if (reportType === undefined) {
      var errorMsg_UnidentifiedReport = 'Unidentified Report';
      throw errorMsg_UnidentifiedReport;
    }

    var inputSheetDimensions = getDimensions(updateInputSheet);
    var inputDataArray = getBacklogArray(
      updateInputSheet,
      inputSheetDimensions
    );
    ...
    if (rowNeeded > 0) {
      ...
      updateDestination
        .getRange(1, 1, rowNeeded, colNeeded)
        .setValues(inputDataArray);
    ...
    var errorMsg_LockLost =
      'Lock was lost. Someone else is updating backlog, please wait.';
